...
The C<ZMQ::LibZMQ4> module is a wrapper of the 0MQ message passing library for Perl. 
...
Note that this is a wrapper for libzmq 4.x. For 2.x, you need to check L<ZMQ::LibZMQ2> and for 3.x, you need to check L<ZMQ::LibZMQ3>

=head1 BASIC USAGE

Please make sure you already have ZMQ::Constants module. If you installed
ZMQ::LibZMQ4 from CPAN via cpan/cpanm, it should have already been installed
...
The received message is an instance of ZMQ::LibZMQ4::Message object, and you can access the content held in the message via the C<zmq_msg_data()> method:
...
=item Using utilities in ZMQ::LibZMQ4
...
See C<zmq_getsockopt()> if you have problems with ZMQ::LibZMQ4 not knowing the type of the option.
...
If C<$message> is a non-ref, creates a new ZMQ::LibZMQ4::Message object via C<zmq_msg_init_data()>, and uses that to pass to the underlying C layer..
...
One of either C<fd> or C<socket> key must exist. C<fd> should contain a UNIX file descriptor. C<socket> should contain a C<ZMQ::LibZMQ4::Socket> socket object.
...
=head1 FUNCTIONS PROVIDED BY ZMQ::LIBZMQ4
...
First, make sure to compile C<ZMQ::LibZMQ4> with debugging on by specifying -g:
...
L<http://github.com/mosconi/p5-ZMQ>
...
Rodrigo Mosconi C<< <perl@mosconi.mat.br> >>
